...
void main() {
  test('featured_recipe_card_stories.dart has updated signature', () {
    final file = File('mobile/lib/widgetbook/components/featured_recipe_card_stories.dart');
    expect(file.existsSync(), isTrue, reason: 'featured_recipe_card_stories.dart does not exist');

    final content = file.readAsStringSync();

    final hasOldSignature = RegExp(r'Widget\s+featuredRecipeCardStories\s*\(\s*BuildContext').hasMatch(content);
    final hasNewReturnType = RegExp(r'List<\s*WidgetbookComponent\s*>\s+buildFeaturedRecipeCardStories\s*\(').hasMatch(content);
